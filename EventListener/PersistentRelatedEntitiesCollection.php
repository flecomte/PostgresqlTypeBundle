...
class PersistentRelatedEntitiesCollection implements Collection, Selectable
{
    ...
    public function __construct(RegistryInterface $registry, $parentEntity, ManyToAny $annotation, \ReflectionProperty $reflectionProperty)
    {
    ...
    private function initialize()
    {
        ...
        if ($this->annotation->table instanceof JoinTable && ($tableName = $this->annotation->table->name) !== null) {
            $tableName = $this->annotation->table->name;
        } else {
            preg_match('`\\\\Entity\\\\(.*)$`', get_class($this->parentEntity), $matches);
            $className = strtolower($matches[1]);
            $tableName = $className.'_'.$this->reflectionProperty->name;
        }
        ...
        $parentClass = get_class($this->parentEntity);
        $parentId = $this->registry->getManagerForClass($parentClass)->getMetadataFactory()->getMetadataFor($parentClass)->getIdentifierValues($this->parentEntity);


        $em = $this->registry->getEntityManager();
        $rsm = new ResultSetMappingBuilder($em);
        $rsm->addScalarResult('related_id', 'related_id', 'jsonb');
        $rsm->addScalarResult('related_class', 'related_class', 'string');

        $query  = $em->createNativeQuery("SELECT related_class, related_id FROM $tableName WHERE parent_id = :parentId", $rsm);
        $query->setParameter('parentId', $parentId, 'json');
        $results = $query->getResult();

        foreach ($results as $data) {
            $count++;
            $entitiesPerClass[$data['related_class']][] = $data['related_id'];
        }
